...
import type { CheckIn, Insight, WeeklyReview } from "../types/ai";
...
const buildPrompt = (
  ...
  const avgStress =
    checkins.reduce((s, c) => s + c.stress, 0) / (checkins.length || 1);
  const avgSleep =
    checkins.reduce((s, c) => s + c.sleepHours, 0) / (checkins.length || 1);
...
export const generateWeeklyReview = async (
  ...
  checkins: CheckIn[],
  insights: Insight[]
): Promise<Omit<WeeklyReview, "id" | "createdAt">> => {
  ...
  const avgSleep =
    checkins.reduce((s, c) => s + c.sleepHours, 0) / checkins.length;
